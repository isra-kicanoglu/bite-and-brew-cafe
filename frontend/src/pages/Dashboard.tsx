...
const Dashboard: React.FC = () => {
  ...
  const { user, stamps, addToCart } = context; 
  ...
  const handleClaimReward = () => {
    if (earnedRewards > 0) {
      ...
      alert("Your free reward coffee has been added to your cart! 🎁");
    }
  };
      ...
      <div className="row justify-content-center">
        <div className="col-md-10 text-center">
          ...
          <div className="card shadow-lg border-0 rounded-5 mb-4 position-relative overflow-hidden" 
            ...
            <div className="position-absolute end-0 top-50 translate-middle-y me-4 opacity-25 d-none d-md-block">
            ...
            <div className="card-body p-0 d-flex flex-column align-items-center justify-content-center position-relative z-1">
              <h5 className="fw-bold mb-4 d-flex align-items-center text-warning">
                <Award size={20} className="me-2" /> Digital Loyalty Card
              </h5>
                ...
                {[...Array(6)].map((_, i) => (
                  <div key={i} className="rounded-circle d-flex align-items-center justify-content-center"
                    style={{ 
                      ...
                      backgroundColor: i < currentProgress ? '#ffc107' : 'rgba(255,255,255,0.1)',
                      border: i < currentProgress ? 'none' : '1px dashed rgba(255,255,255,0.3)',
                    }}
                  >
            ...
            <div className="w-100 py-3" style={{ background: 'rgba(0,0,0,0.2)' }}>
              <p className="mb-0 text-white small opacity-90">
                {earnedRewards > 0 ? (
                  <span className="text-warning fw-bold"><PartyPopper size={16} className="me-1"/> You have a free coffee reward ready!</span>
                ) : (
                  <>Only <span className="text-warning fw-bold">{stampsRemaining}</span> more coffees left until your free one!</>
                )}
              </p>
            </div>
          </div>
              ...
              <div className="d-flex align-items-center justify-content-between flex-wrap gap-3">
                <div className="d-flex align-items-center">
                  ...
                  <div className="text-start">
                    <h5 className="fw-bold mb-0 text-dark">Available Rewards</h5>
                    <p className="text-muted small mb-0">You have <strong>{earnedRewards}</strong> free coffee(s) waiting!</p>
                  </div>
                </div>
                <button onClick={handleClaimReward} className="btn btn-warning fw-bold rounded-pill px-4 shadow-sm">Use 1 Reward</button>
              </div>
            </div>
          )}
